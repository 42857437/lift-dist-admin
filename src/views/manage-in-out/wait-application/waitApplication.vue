<template>
  <div class="main_wrap">
      ...
      <form>
        <div class="selectWrap font12">
          ...
          <div class="column inline-Block">
            ...
            <button class="serachBtn" type="button" @click="add" v-if="handleShow">申请放行</button>
          ...
          <table class="table textAlignL">
            ...
            <tr v-for="(whiteDisInfor,index) in whiteDisInfors" :key="index">
              ...
              <td class="redact" v-if="handleShow">
                <!--<span @click="edit(whiteDisInfor.enterId)">申请放行</span>-->
                <span @click="redact(whiteDisInfor.enterId,whiteDisInfor.carNumber,whiteDisInfor.businessType,whiteDisInfor.outReason,whiteDisInfor.vin,whiteDisInfor.remark)">申请放行</span>
              </td>
    ...
    <!--新增-编辑遮罩层-->
    <addit :popUp="popUp" @changeAddEdit="changeAddEdit" :title="title" @relod="relod" ref="waitAddEdit" :enterId="enterId" :whichApplication="whichApplication" :carNumber="carNumber" :businessType="businessType" :outReason="outReason" :vin="vin" :remark="remark"></addit>
    <!--确认放行遮罩层-->
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        ...
        model:{
          ...
          pageSize:10,//每页行数
          pageNo:1,
          menuId:302,//菜单ID (302 待申请放行303 待审核放行 304待出场）
        },
        whichApplication:'0',//默认搜索部分申请放行按钮，0搜索部分申请放行按钮，1表格中申请放行按钮
    ...
    mounted() {
      // var today=new Date();
      // this.searchModel.entryStart=this.AppUtils.formatDate(today);
      ...
      if(roleId=='3'||roleId=='5'||roleId=='6'||roleId=='7'||roleId=='8'||roleId=='10'||roleId=='11'||roleId=='12'){
    ...
    methods: {
      ...
      searchInfo(){
        // this.$store.state.carNumber=this.model.carNumber;
        // this.$store.state.vin=this.model.vin;
        /*if(!this.enterState){
          var stamp=Date.parse(new Date(this.searchModel.entryStart+' 00:00:00'));
          this.model.entryStart=stamp;
          this.$store.state.entryStart=stamp;
          if(this.searchModel.entryEnd){
            var entryEndStam=this.searchModel.entryEnd.getTime();
            this.$store.state.entryEnd=entryEndStam+86400000;
            this.model.entryEnd=entryEndStam+86400000;//出来两天的数据
            if(this.model.entryStart<=entryEndStam){
              var obj={};
          ...
          if(this.model.entryStart) {
            if(Object.prototype.toString.call(this.model.entryStart)=="[object Date]"){
              var entryStart = this.model.entryStart.getTime();
              this.model.entryStart = entryStart;
            }
      ...
      add(){
        ...
        this.popUp = true
        this.title = '申请放行'
        this.whichApplication='0';
      },
      //表格中申请放行按钮
      redact(enterId,carNumber,businessType,outReason,vin,remark){
        this.vin='';
        this.carNumber='';
        this.businessType='';
        this.outReason='';
        this.remark='';
        this.whichApplication='1';
        this.popUp = true;
        this.title = '申请放行';
        if(enterId){
